...
⑦ 默认收件人邮箱（可选）:
   不指定收件人时自动使用此地址
   例如: boss@company.com — 留空则每次必须手动指定
...
```json
{
  ...
  "default_recipient": "boss@company.com",
  "timeout": 30,
  "_created_at": "2026-04-28T15:00:00"
}
```

> **`default_recipient`**: 可选字段。设置后，发送邮件时若不指定收件人，将自动使用此地址作为默认收件人。留空则每次必须指定。
...
| 要素 | 必填 | 来源 | 示例 |
|------|------|------|------|
| **收件人** | △ | 用户指定或使用默认收件人 | `lisi@company.com` 或 `a@x.com, b@y.com`（未指定时自动使用 `default_recipient`） |
...
> **默认收件人机制**: 如果用户未指定收件人，且配置中存在 `default_recipient`，则自动使用该地址作为收件人，无需额外询问。若两者均无则提示用户输入。
